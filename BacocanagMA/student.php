...
// Fetch students (exclude admins) - ✅ ADDED reward_points
if ($search) {
    $stmt = $pdo->prepare("SELECT id, fname, lname, mname, course, course_level, remaining_session, reward_points
                           FROM students
...
<body>
            ...
            <tbody>
                <?php if (count($students) > 0): ?>
                    <?php foreach ($students as $row): ?>
                        ...
                        $points = $row['reward_points'] ?? 0;
                        $progress = ($points / 3) * 100;
                        ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="reward-bar">
                                    <span style="font-size:0.9rem; font-weight:bold; color:#d4af37;"><?= $points ?>/3</span>
                                    <div class="reward-progress">
                                        <div class="reward-fill" style="width: <?= $progress ?>%; background: <?= $points >= 2 ? '#28a745' : ($points >= 1 ? '#ffc107' : '#e9ecef') ?>;"></div>
                                    </div>
                                    <form method="POST" action="reward_student.php" style="margin:0;"
                                        onsubmit="return confirm('Give 1 reward point to <?= htmlspecialchars($full_name) ?>?');">
                                        <input type="hidden" name="student_id" value="<?= htmlspecialchars($row['id']) ?>">
                                        <button type="submit" class="btn btn-warning">🎁 Reward</button>
                                    </form>
                                </div>
                            </td>
    ...
    <!-- SIT-IN MODAL (Synced with dropdowns) -->
